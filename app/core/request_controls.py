...
class IdempotencyService:
    ...
    def store(self, idem_key: str, method: str, path: str, request_hash: str, response_payload: dict[str, Any]) -> None:
        with self.db.connect() as connection:
            connection.execute(
                """
                INSERT OR REPLACE INTO idempotency_records (idem_key, method, path, request_hash, response_json, created_at)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (
                    idem_key,
