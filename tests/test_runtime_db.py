...
class RuntimeDatabaseTests(unittest.TestCase):
    def test_migrations_create_runtime_tables(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            health = db.health()
            self.assertGreaterEqual(health["migrations"], 3)
            with db.connect() as connection:
                ...
                graph_schema_metadata = connection.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='graph_schema_metadata'").fetchone()
            self.assertIsNotNone(source_registry)
            self.assertIsNotNone(lineage_edges)
            self.assertIsNotNone(graph_schema_metadata)
